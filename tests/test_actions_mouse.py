...
class HScrollCommandTests(unittest.TestCase):
    """TASK: rediseño de scroll (hallazgo de camara real, José, 2026-08-30) -
    scroll horizontal nuevo, via pyautogui.hscroll()."""

    @patch("jarvis.actions.mouse.pyautogui")
    def test_execute_hscrolls_by_amount(self, mock_pyautogui):
        result = HScrollCommand(12).execute()
        mock_pyautogui.hscroll.assert_called_once_with(12)
        self.assertTrue(result.success)

    @patch("jarvis.actions.mouse.pyautogui")
    def test_negative_amount_scrolls_left(self, mock_pyautogui):
        HScrollCommand(-12).execute()
        mock_pyautogui.hscroll.assert_called_once_with(-12)

    @patch("jarvis.actions.mouse.pyautogui")
    def test_execute_failure_is_reported_not_raised(self, mock_pyautogui):
        mock_pyautogui.hscroll.side_effect = RuntimeError("no display")
        result = HScrollCommand(12).execute()
        self.assertFalse(result.success)
        self.assertEqual(result.status, "ERROR")
...
class OtherMouseCommandsAreNotReversibleTests(unittest.TestCase):
    ...
    def test_hscroll_not_reversible(self):
        self.assertFalse(HScrollCommand(12).is_reversible())
